...
export async function getStaticProps() {
  const res = await fetch(`${API_SERVER}/listings`);
  ...
  return (
    <main className="w-full min-h-screen flex flex-col items-center mb-5">
      ...
      <section className="flex flex-col justify-center md:flex-row md:min-h-screen lg:w-10/12">
        ...
        {data.length !== 0 && (
          <div className="w-full h-full grid p-2 grid-cols-1 sm:grid-cols-2 gap-2 grid-flow-row-dense lg:grid-cols-2 xl:grid-cols-3 3xl:grid-cols-4">
            {data.map((listing) => (
